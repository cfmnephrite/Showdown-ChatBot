 ...
 const Moves = {
	...
	grassyterrain: {
		...
		effect: {
			duration: 5,
			durationCallback(source, effect) {
				if (_optionalChain([source, 'optionalAccess', _10 => _10.hasItem, 'call', _11 => _11('terrainextender')])) {
					return 8;
				}
				return 5;
			},
	...
	gravity: {
		...
		effect: {
			duration: 5,
			durationCallback(source, effect) {
				if (_optionalChain([source, 'optionalAccess', _14 => _14.hasAbility, 'call', _15 => _15('persistent')])) {
					this.add('-activate', source, 'ability: Persistent', effect);
					return 7;
				}
				if (_optionalChain([source, 'optionalAccess', _16 => _16.hasItem, 'call', _17 => _17('lightclay')])) {
					return 8;
				}
				return 5;
			},
	...
	healblock: {
		...
		effect: {
			duration: 5,
			durationCallback(target, source, effect) {
				if (_optionalChain([source, 'optionalAccess', _18 => _18.hasAbility, 'call', _19 => _19('persistent')])) {
					this.add('-activate', source, 'ability: Persistent', effect);
					return 7;
				}
				return 5;
			...
			onTryHeal(damage, target, source, effect) {
				if ((_optionalChain([effect, 'optionalAccess', _20 => _20.id]) === 'zpower') || this.effectData.isZ) return damage;
				return false;
			},
		},
		secondary: null,
	...
	lightscreen: {
		...
		effect: {
			duration: 5,
			durationCallback(target, source, effect) {
				if (_optionalChain([source, 'optionalAccess', _21 => _21.hasItem, 'call', _22 => _22('lightclay')])) {
					return 8;
				}
				return 5;
			},
	...
	magiccoat: {
		...
		effect: {
			...
			onStart(target, source, effect) {
				...
				if (_optionalChain([effect, 'optionalAccess', _23 => _23.effectType]) === 'Move') {
	...
	mistyterrain: {
		...
		effect: {
			duration: 5,
			durationCallback(source, effect) {
				if (_optionalChain([source, 'optionalAccess', _26 => _26.hasItem, 'call', _27 => _27('terrainextender')])) {
					return 8;
				}
				return 5;
			},
			...
			onStart(battle, source, effect) {
				if (_optionalChain([effect, 'optionalAccess', _28 => _28.effectType]) === 'Ability') {
	...
	psychicterrain: {
		...
		effect: {
			duration: 5,
			durationCallback(source, effect) {
				if (_optionalChain([source, 'optionalAccess', _29 => _29.hasItem, 'call', _30 => _30('terrainextender')])) {
					return 8;
				}
				return 5;
			},
			...
			onStart(battle, source, effect) {
				if (_optionalChain([effect, 'optionalAccess', _31 => _31.effectType]) === 'Ability') {
	...
	pursuit: {
		...
		onModifyMove(move, source, target) {
			if (_optionalChain([target, 'optionalAccess', _32 => _32.beingCalledBack])) move.accuracy = true;
		},
		onTryHit(target, pokemon) {
	...
	reflect: {
		...
		effect: {
			duration: 5,
			durationCallback(target, source, effect) {
				if (_optionalChain([source, 'optionalAccess', _33 => _33.hasItem, 'call', _34 => _34('lightclay')])) {
					return 8;
				}
				return 5;
			},
	...
	round: {
		...
		onTry(target, source, move) {
			for (const action of this.queue) {
				...
				// @ts-ignore
				if (_optionalChain([action, 'access', _35 => _35.move, 'optionalAccess', _36 => _36.id]) === 'round') {
					// @ts-ignore
					this.queue.prioritizeAction(action, move);
					return;
				}
	...
	safeguard: {
		...
		effect: {
			duration: 5,
			durationCallback(target, source, effect) {
				if (_optionalChain([source, 'optionalAccess', _37 => _37.hasAbility, 'call', _38 => _38('persistent')])) {
					this.add('-activate', source, 'ability: Persistent', effect);
					return 7;
				}
				return 5;
	...
	spacialrend: {
		...
		effect: {
			...
			onEffectiveness(typeMod, target, type, move) {
				...
				if (_optionalChain([move, 'optionalAccess', _39 => _39.flags, 'access', _40 => _40.specialTypeMod]) === type) return;
	...
	suckerpunch: {
		...
		onTry(source, target) {
			const action = this.queue.willMove(target);
			const move = _optionalChain([action, 'optionalAccess', _41 => _41.choice]) === 'move' ? action.move : null;
	...
	tailwind: {
		...
		effect: {
			duration: 4,
			durationCallback(target, source, effect) {
				if (_optionalChain([source, 'optionalAccess', _42 => _42.hasAbility, 'call', _43 => _43('persistent')])) {
					this.add('-activate', source, 'ability: Persistent', effect);
					return 6;
				}
				return 4;
	...
	wideguard: {
		...
		effect: {
			...
			onTryHit(target, source, move) {
				...
				if (_optionalChain([move, 'optionalAccess', _46 => _46.target]) !== 'allAdjacent' && move.target !== 'allAdjacentFoes') {
					return;
				}
				if (move.isZ || move.isMax) {
	...
	wonderroom: {
		...
		effect: {
			...
			onEffectiveness(typeMod, target, type, move) {
				// The effectiveness of "special typeMod" moves isn't reversed
				if (_optionalChain([move, 'optionalAccess', _49 => _49.flags, 'access', _50 => _50.specialTypeMod]) === type) return;
				if (move.id === 'synchronoise' && type === this.activePokemon.getTypes()[0]) return 1;
				if (move && !this.dex.getImmunity(move, type)) return 1;
				return -typeMod;
			},
